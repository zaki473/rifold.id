...
<head>
    ...
    {{-- Library SortableJS (WAJIB ADA UNTUK DRAG & DROP) --}}
    ...
    <style>
        ...
        /* Badge Main Image (Otomatis muncul di gambar pertama) */
...
<body class="bg-gray-50 text-gray-800">
                ...
                <form id="productForm" action="{{ route('store_products') }}" method="POST" enctype="multipart/form-data">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-x-8 gap-y-6">
                        ...
                        <div class="col-span-1">
                            ...
                            {{-- Dropdown Kategori (Sesuai request sebelumnya agar sinkron filter) --}}
                            <div class="relative">
                                <select name="category" class="w-full border border-gray-300 rounded-lg px-4 py-2.5 focus:ring-indigo-500 focus:border-indigo-500 outline-none appearance-none bg-white">
                                    <option value="" disabled selected>Select Category</option>
                                    <option value="flannel" {{ old('category') == 'flannel' ? 'selected' : '' }}>Flannel Shirts</option>
                                    <option value="jacket" {{ old('category') == 'jacket' ? 'selected' : '' }}>Jackets</option>
                                    <option value="polo" {{ old('category') == 'polo' ? 'selected' : '' }}>Polo Shirts</option>
                                    <option value="tshirt" {{ old('category') == 'tshirt' ? 'selected' : '' }}>T-Shirts</option>
                                </select>
                        ...
                        <div class="col-span-1">
                            ...
                            <input type="number" name="price" value="{{ old('price') }}" class="w-full border border-gray-300 rounded-lg px-4 py-2.5 focus:ring-indigo-500 focus:border-indigo-500 outline-none" placeholder="120000">
                        </div>
                        <div class="col-span-1">
                        ...
                        <div class="col-span-1">
                            ...
                            <input type="text" name="color" value="{{ old('color') }}" class="w-full border border-gray-300 rounded-lg px-4 py-2.5 focus:ring-indigo-500 focus:border-indigo-500 outline-none" placeholder="e.g. Black, Navy, #A3C5B5">
                    ...
                    <div class="mt-6">
                        ...
                        <p class="text-xs text-gray-500 mb-3">Foto pertama (paling kiri) otomatis menjadi <strong>Main Image</strong>. Geser (Drag) untuk mengubah urutan.</p>
                        
                        {{-- Area Upload (Klik di sini trigger input file) --}}
                        ...
                        {{-- Container Preview (Tempat Gambar Muncul & Bisa Digeser) --}}
    ...
    {{-- SCRIPT DRAG & DROP LOGIC (INI YANG TADI HILANG) --}}
    <script>
        ...
        fileInput.addEventListener('change', function(e) {
            ...
            renderPreview();
            // Update input file asli (agar kalau disubmit terkirim)
            this.files = dt.files;
        });

        // 2. Fungsi Render Preview ke Layar
        function renderPreview() {
            previewContainer.innerHTML = ''; // Bersihkan container dulu
            ...
            Array.from(dt.files).forEach((file, index) => {
                ...
                reader.onload = function(e) {
                    ...
                    div.innerHTML = `
                        ...
                        <!-- Badge Main Image (Otomatis muncul di urutan pertama) -->
                        <div class="main-tag absolute top-2 left-2 bg-indigo-600 text-white text-[10px] font-bold px-2 py-1 rounded shadow-md">
                            MAIN IMAGE
                        </div>
                        
                        <!-- Tombol Hapus -->
                        <button type="button" onclick="removeImage(${index})" class="absolute top-1 right-1 bg-red-500 text-white rounded-full p-1 opacity-0 group-hover:opacity-100 transition shadow-md hover:bg-red-600">
        ...
        window.removeImage = function(index) {
            ...
            dt = newDt; // Update data transfer
            fileInput.files = dt.files; // Update input file asli
        ...
        new Sortable(previewContainer, {
            animation: 150,
            ghostClass: 'ghost', // Class saat didrag
            onEnd: function() {
                ...
                // Ambil file berdasarkan urutan elemen HTML yang baru (hasil drag)
