    ...
    <div class="container mx-auto px-4 md:px-6 py-10 max-w-6xl pt-28">
                            ...
                            <div class="flex justify-between items-end mt-4">
                                ...
                                <div class="flex items-center border border-gray-300 rounded-lg h-8 md:h-9">
                                    ...
                                    <form action="{{ route('cart.update', $item->id) }}" method="POST">
                                        ...
                                        <button type="submit" class="px-2 md:px-3 text-gray-500 hover:text-black hover:bg-gray-50 h-full rounded-l-lg transition">-</button>
                                    </form>

                                    <input type="number" value="{{ $item->quantity }}" class="w-10 md:w-12 text-center text-sm font-medium border-none focus:ring-0 p-0 text-gray-900" readonly>

                                    {{-- Tombol Tambah --}}
                                    <form action="{{ route('cart.update', $item->id) }}" method="POST">
                                        @csrf @method('PATCH')
                                        <input type="hidden" name="type" value="increase">
                                        <button type="submit" class="px-2 md:px-3 text-gray-500 hover:text-black hover:bg-gray-50 h-full rounded-r-lg transition">+</button>
                                    </form>
                                </div>
                            </div>
                        </div>
                ...
                <div class="bg-white p-6 rounded-xl shadow-sm border border-gray-100 sticky top-24">
                    ...
                    <div class="border-t border-dashed border-gray-300 pt-4 mb-6">
                        <div class="flex justify-between items-center">
                            ...
                            <span class="text-xl font-bold text-black">Rp {{ number_format($totalPrice, 0, ',', '.') }}</span>
                        </div>
                    </div>
        ...
        @else
            ...
            <div class="text-center py-20 bg-white rounded-xl border border-dashed border-gray-200">
                ...
                <a href="{{ route('katalog') }}" class="inline-block bg-black text-white px-6 py-2.5 rounded-lg font-medium hover:bg-gray-800 transition">
                    Mulai Belanja
                </a>
            </div>
        @endif
