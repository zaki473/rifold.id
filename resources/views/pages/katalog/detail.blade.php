...
<body class="bg-white text-gray-800">
    ...
    <main class="w-full pt-28 pb-24">
        <div class="container mx-auto px-4 md:px-6 max-w-6xl">
            ...
            <div class="grid grid-cols-1 lg:grid-cols-2 gap-12 lg:gap-16">
                ...
                {{-- KANAN: INFORMASI & FORM ADD TO CART --}}
                <div class="flex flex-col justify-center">
                    
                    {{-- 
                        FORM BACKEND: 
                        Membungkus Size, Qty, dan Button agar data terkirim ke CartController 
                    --}}
                    <form id="addToCartForm" action="{{ route('cart.add') }}" method="POST">
                        ...
                        {{-- SIZE SELECTION (Data dikirim via Hidden Input) --}}
                        <div class="mb-8">
                            <div class="flex justify-between items-end mb-2">
                                <p class="text-sm font-medium text-gray-900 uppercase tracking-wide">Size</p>
                            </div>
                            
                            {{-- Input Hidden untuk menyimpan Size yang dipilih --}}
                                ...
                                @if($product->size)
                                    @foreach(explode(',', $product->size) as $s)
                                        <button type="button" data-size="{{ trim($s) }}"
                                            class="size-btn min-w-[3.5rem] h-12 border border-gray-200 rounded-lg text-sm font-medium text-gray-700 hover:border-black transition-all focus:outline-none bg-white">
                                            {{ trim($s) }}
                                        </button>
                                    @endforeach
                                @else
                        ...
                        <div class="flex flex-col sm:flex-row gap-4">
                            <div class="flex items-center border border-gray-300 rounded-lg h-12 w-fit sm:w-auto">
                                <button type="button" id="decreaseQty" class="px-4 text-gray-500 hover:text-black transition">-</button>
                                {{-- Input Name='quantity' agar terbaca controller --}}
                                <input name="quantity" id="quantity" type="number" min="1" value="1" class="w-12 text-center text-gray-900 font-medium focus:outline-none h-full bg-transparent">
                                <button type="button" id="increaseQty" class="px-4 text-gray-500 hover:text-black transition">+</button>
                            </div>
                            
                            {{-- Button Submit Form --}}
                            <button type="button" id="addToCartBtn" class="flex-1 bg-black text-white h-12 rounded-lg font-semibold hover:bg-gray-800 transition active:scale-[0.99] shadow-lg">
                                ADD TO CART
                            </button>
                        </div>
                    </form>
    ...
    <div id="cartPopup" class="hidden fixed inset-0 z-50 flex items-center justify-center p-4">
        ...
        <div class="relative bg-white rounded-2xl shadow-2xl w-full max-w-sm p-6 transform transition-all scale-100">
            ...
            <div class="space-y-3">
                <a href="{{ route('cart') }}" class="block w-full rounded-xl bg-black px-4 py-3.5 text-sm font-semibold text-white shadow-sm hover:bg-gray-800 text-center transition">Lihat Keranjang</a>
                <button onclick="closeCartPopup()" class="block w-full rounded-xl bg-white px-4 py-3.5 text-sm font-semibold text-gray-900 shadow-sm ring-1 ring-inset ring-gray-300 hover:bg-gray-50 text-center transition">Lanjut Belanja</button>
            </div>
        </div>
    </div>
    ...
    <input type="hidden" id="success-add-to-cart" value="{{ session('success_add_to_cart') ? 'true' : 'false' }}">
    ...
    <script>
        ...
        const sizeInput = document.getElementById('selectedSizeInput'); // Input hidden untuk backend
        ...
        sizeBtns.forEach(btn => {
            btn.addEventListener('click', (e) => {
                ...
                // Simpan size ke variable dan input hidden
        ...
        if (qtyInput && inc && dec) {
            inc.onclick = () => qtyInput.value++;
            dec.onclick = () => qtyInput.value = Math.max(1, qtyInput.value - 1);
        }
        ...
        // --- CEK SESSI FLASH DARI LARAVEL ---
        // Jika backend berhasil simpan, dia akan redirect balik dengan session 'success_add_to_cart'
        // Kita tangkap session itu untuk memunculkan popup
        const successAddToCart = document.getElementById('success-add-to-cart').value === 'true';
        if (successAddToCart) {
